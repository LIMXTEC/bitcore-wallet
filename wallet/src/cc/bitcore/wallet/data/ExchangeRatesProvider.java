...
package cc.bitcore.wallet.data;
...
import cc.bitcore.wallet.util.GenericUtils;
...
public class ExchangeRatesProvider extends ContentProvider {
    ...
    private static final HttpUrl COINMARKET_URL = HttpUrl
            .parse("https://api.coinmarketcap.com/v1/ticker/Bitcore/");
    private static final String COINMARKET_SOURCE = "CoinMarketCap.com";
    ...
    private Long getCoinValueInBTC() {
      final Request.Builder request = new Request.Builder();
      request.url(COINMARKET_URL);
      ...
      try {
          ...
          if (response.isSuccessful()) {
              ...
              price = (long) ((double)head.getDouble("price_btc") * 100000.0);
          } else {
              log.warn("http status {} when fetching BTX price from {}", response.code(), COINMARKET_URL);
          }
      } catch (final Exception x) {
          log.warn("problem fetching exchange rates from " + COINMARKET_URL, x);
      }
      return price;
    }
    ...
    private Map<String, ExchangeRate> requestExchangeRates() {
        ...
        Long btxPrice = getCoinValueInBTC();
        final Request.Builder request = new Request.Builder();
